...
static void *thread_udp_send (void *arg)
{
    ...
    if (logtype_tf->getUdpRemoteEndpoint() != NULL)
        udp_remote_endpoint = *(logtype_tf->getUdpRemoteEndpoint());

    boost::asio::ip::udp::socket* 
        udp_socket = logtype_tf->getUdpSocket();
...
void LogTypeTimeFolder::setCurrentLogFile()
{
    ...
    try {
        ...
        if ( getCSVWorker()->setFile(getCurrentLogFile()) )
...
CSVWorkerPtr LogTypeTimeFolder::getCSVWorker()
{
    return __csvWorker;
}
...
void LogTypeTimeFolder::run()
{
    ...
    getCSVWorker()->setDelimeter(getDelimeter());

    setUdpRemoteEndpoint();
    setUdpSocket();
    ...
    switch(typeId)
    {
        ...
        case 0:
            ...
            if (!isThreadRun()) {
                ...
                if (getCSVWorker())
                    getCSVWorker()->setFile(getCurrentLogFile());
        ...
        case 1:
            ...
            else
            {
                ...
                if (__csvSetMaxIndex) {
                    if (getCSVWorker())
                        getCSVWorker()->setMaxSize(index + 1);
                }
